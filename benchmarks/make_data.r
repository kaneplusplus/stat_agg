...
train_inds = sample(1:nrow(x), round(0.3*nrow(x)))
...
# 5 Linear regressors, 5 variables a piece.

lm_fits = foreach(i=1:5) %do% {
  regs = sample(regressor_names, 5)
  form = as.formula( paste("DepDelay", paste(regs, collapse=" + "), sep=" ~ "))
  lm(form, train_data)
}

poisson_fits = foreach(i=1:5) %do% {
  regs = sample(regressor_names, 5)
  form = as.formula( paste("DepDelay", paste(regs, collapse=" + "), sep=" ~ "))
  glm(form, train_data, family=poisson)
}

fitted_values = c(lapply(lm_fits, function(x) predict(x, train_data)),
                  lapply(poisson_fits, function(x) predict(x, train_data)))
...
# Test data
fitted_df = as.data.frame(c(lapply(lm_fits, function(x) predict(x, test_data)),
                            lapply(poisson_fits, 
                                   function(x) predict(x, test_data))))
